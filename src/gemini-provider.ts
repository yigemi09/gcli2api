 ...
 */
const OAUTH_CLIENT_ID: string = '681255809395-oo8ft2oprdrnp9e3aqf6av3hmdib135j.apps.googleusercontent.com';
 ...
 */
const OAUTH_CLIENT_SECRET: string = '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';

/**
 * Gemini Code Assist API 的基础端点。
 */
const CODE_ASSIST_ENDPOINT: string = 'https://cloudcode-pa.googleapis.com';
...
class GeminiCliProvider {
  ...
  public async ensureAuthenticated(): Promise<void> {
    ...
    if (expiryDate && expiryDate <= (Date.now() + 60 * 1000)) {
      ...
      try {
        ...
        await fs.writeFile(
          ...
          JSON.stringify(this.authClient.credentials, null, 2),
          'utf-8'
        );
    ...
    if (this.authClient.credentials.access_token) {
        try {
            ...
            broadcastLog('error', `预热项目ID时失败: ${(error as Error).message}`);
  ...
  private async callEndpoint(method: string, body: any): Promise<any> {
    ...
    try {
      ...
      throw new Error(`API调用失败: ${method}。错误信息: ${(error as Error).message}`);
    }
  }
  ...
  public async discoverProjectId(): Promise<string> {
    ...
    try {
      ...
      const onboardRequest = {
        tierId: loadResponse.allowedTiers?.find((t: any) => t.isDefault)?.id || "free-tier",
        cloudaicompanionProject: "default",
        metadata: clientMetadata,
      };
  ...
  public async createMessage(
    ...
    modelId: string = 'gemini-1.5-pro-001',
    ...
    const response = await this.authClient.request<Readable>({
      url,
      method: 'POST',
      params: { alt: 'sse' },
